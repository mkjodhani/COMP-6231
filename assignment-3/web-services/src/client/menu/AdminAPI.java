...
public class AdminAPI {
    ...
    public AdminAPI(String adminId) {
        ...
        this.admin = getAdminService(Role.getLocationPrefix(adminId).toLowerCase());
    ...
    public static shared.Admin getAdminService(String prefix){
        try{
            System.out.println("prefix::"+prefix);
            int port = CentralRepository.locationPorts.get(prefix);
            URL url = new URL(String.format("http://localhost:%d/ws/admin?wsdl",port));

            QName qname = new QName("http://web.theatre.server/", "AdminServiceService");

            Service service = Service.create(url, qname);
            shared.Admin admin = service.getPort(shared.Admin.class);
            return admin;
        }catch (Exception e){
            e.printStackTrace();
            return null;
