...
//                return Message.getErrorMessage(("No movie found by name of " + movieName));
    ...
    public static final String addMovieTickets(String command) {
        ...
        for(String serverPrefix : Theatre.locationPorts.keySet()){
            ...
            {
                Message localMessage = Message.generateMessageFromString(Actions.getBookingSchedule(Commands.getBookingScheduleCommand(customerID)));
                ...
                try {
                    Message remoteMessage = Message.generateMessageFromString(Query.executeCommandByServerPrefix(Commands.getBookingScheduleCommand(customerID),serverPrefix));
        ...
        if(tickets.size() > 3){
            response = Message.getErrorMessage("You have already booked tickets three times for the next week.Please try to book a ticket later.");
        }else{
            response = Message.generateMessageFromString(addMovieTicketsByLocal(customerID, movieID, movieName, numberOfTickets));
        }
        return response.getMessage();
    }
    ...
    public static final String getAvailableSeatsByMovie(String command) {
        String[] args = command.split(Commands.DELIMITER);
        String movieID = args[1], movieName = args[2];
        LOGGER.log(Level.SEVERE,String.format("Add movie tickets using UDP command:%s",command));
        int totalTickets = Movie.getAvailableTicketsForSlot(movieName,movieID);
        Message response = Message.getSuccessMessage(String.valueOf(totalTickets));
        return response.getMessage();
    }
    public static final String getCustomerSeatsByMovieSlot(String command) {
        String[] args = command.split(Commands.DELIMITER);
        String customerID = args[1] ,movieID = args[2], movieName = args[3];
        LOGGER.log(Level.SEVERE,String.format("Add movie tickets using UDP command:%s",command));
        int totalTickets = Movie.getTotalTicketsFromCustomerID(customerID,movieName,movieID);
    ...
    public static final String addMovieTicketsByLocal(String customerID, String movieID, String movieName, int numberOfTickets) {
        ...
        Movie movie = Theatre.getMovies().get(movieName);
        if(movie == null){
            response = Message.getErrorMessage("Movie is not listed for " + movieName + " at " + Theatre.getMetaData().getLocation() + ".");
        }
        else{
            Slot selectedSlot = movie.getSlots().get(movieID);
            if(selectedSlot == null){
                response = Message.getErrorMessage("Movie slot is not listed for " + movieID + "("+movieName+") at " + Theatre.getMetaData().getLocation() + ".");
            }
            else{
                if(selectedSlot.addTicket(customerID,numberOfTickets)){
                    response = Message.getSuccessMessage(numberOfTickets+ " tickets are booked for " + movieName + " at " + Theatre.getMetaData().getLocation() + ".");
                }
                else{
                    response = Message.getErrorMessage(String.format("%d tickets are not available for %s(%s) at %s.", numberOfTickets, movieName,selectedSlot.getSlotId(), Theatre.getMetaData().getLocation()));
                }
            }
        }
        return response.getMessage();
    }
    public static final String getAvailableSeatsByMovieByLocal(String movieID, String movieName) {
        Message response ;
        Movie movie = Theatre.getMovies().get(movieName);
        if(movie == null){
            response = Message.getErrorMessage("Movie is not listed for " + movieName + " at " + Theatre.getMetaData().getLocation() + ".");
            ...
            if(selectedSlot == null){
                ...
                response = Message.getSuccessMessage(String.format("%s %s %d",movieID,movieName,selectedSlot.getAvailability()));
            }
        }
        return response.getMessage();
    }
    public static final String cancelMovieTicketsByLocal(String customerID, String movieID, String movieName, int numberOfTickets) {
        ...
        Movie movie = Theatre.getMovies().get(movieName);
        if(movie == null){
            response = Message.getSuccessMessage("Movie is not listed for " + movieName + " at " + Theatre.getMetaData().getLocation() + ".");
        }
        else{
            Slot selectedSlot = movie.getSlots().get(movieID);
            if(selectedSlot == null){
                ...
                if(selectedSlot.cancelTickets(customerID,numberOfTickets)){
                    response = Message.getSuccessMessage(numberOfTickets+ " tickets are cancelled  for " + movieName + " at " + Theatre.getMetaData().getLocation() + ".");
                }
                else{
                    response = Message.getErrorMessage(numberOfTickets+ " tickets are not booked for " + movieName + " at " + Theatre.getMetaData().getLocation() + ".");
                }
            }
        }
        return response.getMessage();
    ...
    public static String addMovieSlotByLocal(String movieID, String movieName, int bookingCapacity) throws Exception {
        ...
        if(movie == null){
            ...
            if(alreadyExist){
                message = Message.getSuccessMessage(String.format("Movie slot for %s for %s is successfully updated with %d capacity.",movieName,movieID,bookingCapacity));
            }
            else{
                message = Message.getSuccessMessage(String.format("Movie slot for %s for %s is successfully created with %d capacity.",movieName,movieID,bookingCapacity));
            }
        }
        return message.getMessage();
    }
        ...
        if(movie == null){
            ...
            if(deletedSlot == null){
                ...
                if(nextSlotID == null) {
                    ...
                    response = Message.getErrorMessage("No slots available to transfer the booked seats of " + movieID + " for " + movieName + ".");
                    ...
                    response = Message.getSuccessMessage(String.format("Movie slot for %s for %s is successfully deleted.Also the next slot(%s) is extended to %d tickets.",movieName,movieID,nextSlotID,deletedSlot.getBookedTickets()));
                }
            }
        }
